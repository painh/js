...
var g_btnRect = [{ x : config['width'] - 100,
					y : 0,
					width : 100,
					height : actorHeight * 3,
					text : "즉시 승부"},
...
var SceneIngame = function()
{
	...
	this.Restart = function()
	{
		...
		for(var i = 0; i < g_midLinePlayerActorList.length; ++i)
		{
			...
			actor.type = randomRange(playerStart, playerEnd)
		...
		for(var i = 0; i < g_enemyTable.length; ++i)
		{
			...
			for(var j = 0; j < g_enemyTable[i].length; ++j)
			{
				var actor = new Actor()
				actor.x = g_stageStartX + i * actorWidth
				actor.y = (j + 2) * actorHeight + actorHeight * 3
				actor.renderX = actor.x
				actor.renderY = actor.y
				actor.type = randomRange(enemyStart, enemyEnd)
	...
	this.Update = function()
	{
		...
		for(var i = 0; i < g_midLineEnemyActorList.length; ++i)
		{
			...
			g_midLineEnemyActorList[i].y = (5 * actorHeight) + 8 + actorHeight

			for(j = g_enemyTable[i].length -1; j >= 1; --j)
			{
				g_enemyTable[i][j] = g_enemyTable[i][j-1]
				g_enemyTable[i][j].y = (j+1) * actorHeight + actorHeight * 2
			}
				
			var actor = new Actor()
			actor.x = g_stageStartX + i * actorWidth
			actor.y = actorHeight + actorHeight * 2
			actor.renderX = actor.x
			actor.renderY = actor.y
			actor.type = randomRange( enemyStart, enemyEnd)
...
//						g_bounus += Renderer.currentTime - g_midLineTimer
		...
		if(MouseManager.Clicked && !out && !forceClear)
		{
			for(var idx in g_currentBlock)
			{
...
//					g_lose++
	...
	this.Render = function()
	{
		...
		Renderer.Rect(g_stageStartX , actorHeight * 8 - 22, stageWidth * actorWidth, 5)
		Renderer.Rect(0 , 20, 100, 5)

		Renderer.SetColor('rgba(255, 0, 0, 1)')
		Renderer.Rect(g_stageStartX , actorHeight * 8 - 22 + 1, (Renderer.currentTime - g_midLineTimer) / g_roundMaxSec *  stageWidth * actorWidth, 3)
		Renderer.SetColor('rgba(0, 255, 0, 1)')
		Renderer.Rect(0 , 20, 100, 5)

		Renderer.SetColor('rgba(255, 255, 255, 1)')		
		Renderer.Text(0, 0, "Level " + g_level + ", coin" + g_coin )
